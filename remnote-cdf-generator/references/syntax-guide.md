...
**For Section 9 Practice MCQs** (no PYQ anchor needed — predictive questions based on gap analysis):
```markdown
- *~prediction basis* ;- Questions generated by gap analysis: untested dimensions, gap years in PYQ record, post-last_asked developments, recurring trap patterns.
...
- Which of the following best describes demand-pull inflation? >>A)
  ...
  - ✅ Explanation #[[Extra Card Detail]]
    ...
    - 📊 Prediction basis: "Cost-push vs demand-pull distinction" untested since 2018 (6-year gap); statement-based pattern matches UPSC's style for Economy topics
```
...
Classification Landscape descriptors (use inside Classification Landscape blocks only):
  *~classification count* ;; → How many independent bases exist (atomic, directly testable — `;;`)
  *~UPSC note* ;-            → Which bases/types tested vs untested (`;-` not `;;` — bundles multiple planning facts, not atomic)
  *~all types* ;;>1.         → LIST-ANSWER enumeration: "All types by [basis]?" → recall ordered list
  *~basis* ;;                → Which classification basis this type belongs to (cross-basis flashcard — `;;`)
  *~trigger condition* ;-    → What conditions produce/cause this type (`;-` by default — causal context, not a direct recall target; upgrade to `;;` ONLY if topic-map confirms UPSC has tested this trigger directly)
  *⚡ [Basis] types — side-by-side* ;- → Inline mini-table comparing types within one basis (`;-` = scaffold, never a flashcard)
  *~[Basis N] count* ;;      → Summary: "[N] types: Type A | Type B | Type C" (atomic, testable — `;;`)
```
...
Enumerate all types under a basis?        Use ;;>1.  (list-answer descriptor)
  → e.g.: *~all types* ;;>1. with Type A / Type B / Type C as children
  → generates: "All types?" → must recall complete ordered list
  → use inside Classification Landscape blocks in Section 5
Which basis does this type belong to?     Use *~basis* ;; [basis name and criterion]
```
